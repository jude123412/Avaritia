...
public class CompressorRecipeWrapper implements IRecipeWrapper {
    ...
    public CompressorRecipeWrapper(ICompressorRecipe recipe) {
        ...
        singularity = helper.createAnimatedDrawable(AvaritiaJEIPlugin.static_singularity,
                Math.max(recipe.getCost() / 16, 1), StartDirection.BOTTOM, false);
    }
    ...
    public void drawInfo(Minecraft minecraft, int recipeWidth, int recipeHeight, int mouseX, int mouseY) {
        ...
        minecraft.fontRenderer.drawString(thing, recipeWidth / 2 - minecraft.fontRenderer.getStringWidth(thing) / 2, 31,
                0x404040);
    }
}
